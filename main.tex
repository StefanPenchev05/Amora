...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2.5cm}[Authentication Flow Diagram]\\User Registration/Login → bcrypt password verification → JWT access token (short-lived) + refresh token (long-lived) → Expo SecureStore storage → Middleware validation → User context resolution\vspace{2.5cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2cm}[System Framework Diagram]\\Illustrating the four continuous cycles: Shared Organization, Emotional Awareness, Intentional Appreciation, and Memory Building\vspace{2cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2cm}[Relationship Setup Flow]\\User A registers → creates invite → generates invite code → User B registers → enters invite code → RelationshipInvite status: sent → User B accepts → Relationship created with partner\_a\_id < partner\_b\_id → both users share relationship\_id\vspace{2cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{3.5cm}[Complete System Architecture]\\React Web Client (Vite) ↔ REST API ↔ Go Backend (Chi Router) ↔ MySQL Database\\React Native Mobile (Expo) ↔ REST API (with SQLite cache)\\Components: JWT Auth Service, GORM Repositories, Domain Services, Use Cases, HTTP Handlers, Middleware Pipeline (CORS, Logging, Auth, Recovery)\vspace{3.5cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{3cm}[Clean Architecture Diagram]\\Backend: Domain Layer (Entities, Value Objects, Events) ← Application Layer (Use Cases, Interfaces) ← Infrastructure Layer (GORM, JWT, HTTP) ← Presentation Layer (Handlers, DTOs, Routes)\\Mobile: Domain (Entities, VOs, Repos) ← Use Cases ← Infrastructure (HTTP Client, SQLite) ← Presentation (ViewModels, Screens)\vspace{3cm}}}
