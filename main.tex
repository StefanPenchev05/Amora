...
\begin{itemize}
    ...
    \item \textbf{Database}: MySQL with GORM ORM and comprehensive migration system
...
Authentication is implemented using a JWT-based dual-token system with HS256 signing:\begin{itemize}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2.5cm}The authentication flow begins when users register or log in. The system verifies credentials using bcrypt, then generates two tokens: a short-lived access token for API requests and a long-lived refresh token for session renewal. On mobile devices, both tokens are stored securely using Expo SecureStore. For each API request, the middleware validates the access token and resolves the user context from the token claims.\vspace{2.5cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2cm}The framework operates through four interconnected cycles that support different aspects of the relationship. Shared Organization provides structure through calendars and milestones. Emotional Awareness enables mood tracking and affirmations for mutual support. Intentional Appreciation facilitates love notes and recognition of achievements. Memory Building preserves meaningful moments over time. These cycles work together to create a comprehensive relationship support system.\vspace{2cm}}}
  \caption{Amora System Framework: Four continuous cycles connecting partners through a unified relationship space}
  \label{fig:amora-system-overview}
\end{figure}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2cm}The pairing process starts when the first partner registers and creates an invitation code. The second partner then registers and enters this code, which updates the invitation status to pending. Once the second partner accepts, the system creates the relationship record with normalized partner ordering (ensuring partner\_a\_id is always less than partner\_b\_id). Both partners now share the same relationship\_id, which scopes all their content access.\vspace{2cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{3.5cm}The architecture consists of three main layers. The presentation layer includes a React web client built with Vite and a React Native mobile app using Expo. Both clients communicate with the backend through a REST API. The application layer contains the Go backend with a Chi router, implementing domain services, use cases, and HTTP handlers. The middleware pipeline handles CORS, logging, authentication, and error recovery. The infrastructure layer includes JWT authentication services, GORM repositories for database access, and a MySQL database for data persistence. The mobile client also maintains a local SQLite cache for offline functionality.\vspace{3.5cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{3cm}Both backend and mobile follow clean architecture principles with clear dependency direction. In the backend, the domain layer contains entities, value objects, and domain events. The application layer defines use cases and interfaces. The infrastructure layer implements these interfaces using GORM for database access, JWT for authentication, and HTTP utilities. The presentation layer handles HTTP requests through handlers, DTOs, and route configuration. The mobile app follows a similar pattern: the domain layer defines entities and repository interfaces, use cases orchestrate business logic, infrastructure provides HTTP clients and SQLite caching, and the presentation layer manages ViewModels and screen components.\vspace{3cm}}}
  \caption{Multi-Layer Architecture: Domain-Driven Design in backend, Clean Architecture in mobile with dependency inversion}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{3cm}The database schema centers around the Relationship table, which connects two User records. The Users table stores credentials, profiles, and settings. RelationshipInvites manages the pairing flow between partners. Content tables like CalendarEvents, MoodCheck, CoupleNote, Posts, and MediaFiles all reference the Relationship table through foreign keys. CHECK constraints enforce data integrity, including the normalized partner ordering rule. The schema includes over 40 composite indexes optimized for relationship-scoped queries and time-range filtering.\vspace{3cm}}}
...
The current implementation delivers a production-ready architecture with:
\begin{itemize}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2cm}The mobile registration interface features animated text fields that provide real-time feedback as users type. A password strength indicator helps users create secure credentials by showing strength levels based on character variety and length. The design uses gradient buttons and glass-morphism effects for a modern, approachable aesthetic. Behind the interface, the ViewModel pattern manages form state and validation logic, separating presentation concerns from business logic.\vspace{2cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2cm}The calendar interface displays events that both partners can see and edit. Events are color-coded by category (dates, appointments, anniversaries, tasks) for quick visual parsing. Recurring events show special indicators and follow the RFC 5545 RRULE standard, supporting complex patterns like weekly date nights or monthly celebrations. All times are displayed in the user's local timezone while being stored in UTC, ensuring consistency when partners are in different locations.\vspace{2cm}}}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.93\linewidth}{\centering\vspace{2cm}The mood tracking interface uses a simple 1-5 scale for logging emotional state, with optional notes for additional context. Users can view their historical mood patterns through visualization charts, helping them identify trends over time. Privacy controls let each partner decide whether to share their current mood with the other or keep it private. The design avoids gamification elements like streaks or comparisons that might create pressure or anxiety.\vspace{2cm}}}
...
\begin{itemize}
    \item Authentication endpoints: <100ms (bcrypt work factor balanced for security/performance)
    \item Calendar event queries: <50ms with indexed relationship+time range lookups
    \item Mood entry creation: <30ms with single-table inserts
    \item Structured logging (slog) with minimal overhead for production debugging
\end{itemize}

\textbf{Mobile App Performance}:
\begin{itemize}
    \item SQLite local cache reduces API calls by 60\% for repeated views
    \item Expo SecureStore provides hardware-backed encryption on supported devices
    \item React Native Reanimated for 60fps animations without JS thread blocking
    \item Optimistic UI updates provide instant feedback before server confirmation
\end{itemize}
...
\begin{itemize}
    \item Unit tests for domain logic (Value Objects, Entity methods)
    \item Repository integration tests with test database
    \item HTTP handler tests with mocked dependencies
    \item JWT token validation and refresh flow tests
\end{itemize}

\textbf{Mobile Testing}:
\begin{itemize}
    \item Use Case unit tests with mocked repositories
    \item ViewModel logic testing with state management verification
    \item Component rendering tests with React Test Renderer
    \item SecureStore integration tests for token persistence
\end{itemize}

\textbf{Security Testing}:
\begin{itemize}
    \item Attempted cross-couple data access (blocked by repository filtering)
    \item ID enumeration attacks (prevented by relationship-scoped queries)
    \item Token tampering detection (JWT signature verification)
    \item Password strength enforcement (Value Object validation)
\end{itemize}
